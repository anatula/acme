...
const VALID_DOMAINS = ["example.com"];

const FALLBACK_KEY = fs.readFileSync(
  path.join(__dirname, "..", "fallback-certificate", "fallback.key")
);
const FALLBACK_CERT = fs.readFileSync(
  path.join(__dirname, "..", "fallback-certificate", "fallback.crt")
);
...
async function getCertOnDemand(client, servername, attempt = 0) {
  ...
  const cert = await client.auto({
    ...
    challengeCreateFn: async (authz, challenge, keyAuthorization) => {
      alpnResponses[authz.identifier.value] =
        await acme.crypto.createAlpnCertificate(authz, keyAuthorization);
    },
    ...
    const alpnResponder = https.createServer({
      ...
      SNICallback: async (servername, cb) => {
        try {
          ...
          log(
            `Found ALPN certificate for ${servername}, serving secure context`
          );
          cb(
            null,
